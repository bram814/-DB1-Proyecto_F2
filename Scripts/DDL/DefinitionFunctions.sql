...
create or replace package number_to_words is
    function N_TO_LETTERS(numero in number) return varchar2;
end number_to_words;

create or replace package body number_to_words is

    -- definicion funcion local
    function numero_menor_mil(numero in number) return varchar2;
   
    -- implementacion
    function N_TO_LETTERS(numero in number) return varchar2 is
